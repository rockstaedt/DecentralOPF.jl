...
function optimize_subproblem(generator::Generator)
    ...
    @constraint(
        ...
        vcat(
            ...
            sum(
                injection[:, t].data
                .-  admm.lambdas[admm.iteration][node_id, t] / admm.gamma
            )
        )
        in SecondOrderCone()
    )
    ...
    @objective(
        ...
        sum(G[t] * generator.marginal_costs
            ...
            + 1/2 * penalty_term_R_ref_var[t]
            + 1/2 * penalty_term_R_cref_var[t]
            + 1/2 * (G[t] - prev_G[t])^2
            for t in admm.T)
    )
...
begin
    admm = ADMM(0.0002, nodes, generators, storages, lines)
